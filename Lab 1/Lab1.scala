...
object Lab1 extends jsy.util.JsyApplication with jsy.lab1.Lab1Like {
  ...
  def repeat(s: String, n: Int): String = {
    ...
    n match {
      ...
      case 0 => ""
        // return the original string to be added
      case 1 => s
        // repeat until n = 1, then recurse and return to concat to back
      case n =>
  ...
  def sqrtStep(c: Double, xn: Double): Double = {
    // copied the equation from packet
    xn - (((xn * xn) - c) / (2 * xn))
  }
  ...
  def repOk(t: SearchTree): Boolean = {
    def check(t: SearchTree, min: Int, max: Int): Boolean = t match {
      case Empty => true
      case Node(l, d, r) => {
        // if we go left, all nodes are >= to minimum, opposite for right
        // check both sides to see if it passes
        if ((d < max) && (d >= min))
  ...
  def deleteMin(t: SearchTree): (SearchTree, Int) = {
    require(t != Empty)
    (t: @unchecked) match {
      case Node(Empty, d, r) => (r, d) //left node empty then current node must be min
      case Node(l, d, r) => {//if left not empty go further
        val (l1, m) = deleteMin(l) //
        (Node(l1,d,r), m) } //updated tree, deleted node
    }
  }

  // Node extends the search tree
